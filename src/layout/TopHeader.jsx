...
export default function TopHeader({ role }) {
    ...
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-5 py-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-right">
          <p className="text-sm font-semibold text-slate-800">{activeRoleConfig.profileName}</p>
          <p className="text-xs text-slate-500">{activeRoleConfig.profileEmail}</p>
        </div>
      </div>
    </header>
  )
